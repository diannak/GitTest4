package comstudy.android.ex32_xmljsonex;
//제이슨 쓰는 일이 많다.
...
public class MainActivity extends AppCompatActivity {
    ...
    private void getJsonData3() {
        ...
        try {
            ...
            String attributed = menuObject.getString("id");
            String attributeValue = menuObject.getString("value");
            JSONObject popupObject = menuObject.getJSONObject("popup");

            JSONArray menuitgemArray = popupObject.getJSONArray("menuitem");

            for (int i = 0; i < menuitgemArray.length(); i++) {
                Log.d(TAG, "value:" + menuitgemArray.getJSONObject(i).getString("value"));
                Log.d(TAG, "onclick:" + menuitgemArray.getJSONObject(i).getString("onclick"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
